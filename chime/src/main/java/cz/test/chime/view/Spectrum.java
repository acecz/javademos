...
public class Spectrum extends JComponent implements Runnable {
	 ...
	 * 
	 * @param amp
	 *            amp[0..FFT.FFT_N/2-1]为频谱"幅值"(用复数模的平方)。
	 */
	private void drawHistogram(float[] amp) {
		...
		for (; i != band; i++, x += deltax) {
			...
			/*
			 * maxAmp转换为用对数表示的"分贝数"y: y = (int) Math.sqrt(maxAmp); y /=
			 * FFT.FFT_N; //幅值 y /= 8; //调整 if(y > 0) y = (int)(Math.log10(y) *
			 * 20 * 2);
			 * 
