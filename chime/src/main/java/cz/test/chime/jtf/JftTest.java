...
public class JftTest {
	public static double[] data = { 1, 2, 3, 2, 1, 2, 3, 2, 1, 2, 3, 2, 1, 2, 3, 1 };
	...
	private static void testDoubleFFT_1D1() {
		double[] sampleDate = new double[len * 2];
		for (int i = 0; i < len; i++) {
			sampleDate[i * 2] = data[i];
		}
		DoubleFFT_1D transfer = new DoubleFFT_1D(len);
		transfer.complexForward(sampleDate);
		System.out.println(Arrays.toString(sampleDate));
	}

	static void testDoubleDCT_1D() {
		DoubleDCT_1D dct = new DoubleDCT_1D(len);
		double[] tmp = new double[len];
		System.arraycopy(data, 0, tmp, 0, len);
		dct.forward(tmp, false);
		System.out.println(Arrays.toString(tmp));
		System.arraycopy(data, 0, tmp, 0, len);
		dct.forward(tmp, true);
		System.out.println(Arrays.toString(tmp));
	}

	static void testDoubleFFT_1D() {
		DoubleFFT_1D transfer = new DoubleFFT_1D(len);
		double[] tmp = new double[len];
		System.arraycopy(data, 0, tmp, 0, len);
		transfer.complexForward(data);
		System.out.println(Arrays.toString(tmp));
	}
	...
	public static void aaaa(String[] args) {
		...
		for (double phase = 0; samplePos < lengthInSecs * SAMPLERATE
				&& samplePos < samplesNum; phase += (2 * Math.PI * frequency) / SAMPLERATE) {
		...
		// we need to initialize a buffer where we store our samples as complex
		// numbers. first value is the real part, second is the imaginary.
		...
		// fft resolution (number of bins) is samplesNum, because we initialized
		// with that value
		...
		try {
			...
			int max_i = -1;
			double max_fftval = -1;
			for (int i = 0; i < fftData.length; i += 2) { // we are only looking
														  // at the half of the
														  // spectrum
				double hz = ((i / 2.0) / fftData.length) * SAMPLERATE;
				outputStream.write(i + ".\tr:" + Double.toString((Math.abs(fftData[i]) > 0.1 ? fftData[i] : 0)) + " i:"
						+ Double.toString((Math.abs(fftData[i + 1]) > 0.1 ? fftData[i + 1] : 0)) + "\t\t" + hz
						+ "hz\n");

				// complex numbers -> vectors, so we compute the length of the
				// vector, which is sqrt(realpart^2+imaginarypart^2)
