...
public class TestMain2 {
	public static void main(String[] args) throws Exception {
		for (int i = 0; i < 8; i++) {
			final int from = i * 10000;
			new Thread(() -> {
				try {
					getWowSpells(from, from + 10000);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}).start();
	...
	private static void getWowSpells(int from, int to) throws Exception {
		PrintWriter pw = null;
		try {
			pw = new PrintWriter("wowSpells-" + from + "-" + to + ".html");
	...
	private static void appendContent(Element table, int from, int to) {
		String utlFmt = "http://db.duowan.com/wow/spell-%d.html";
		long start = System.currentTimeMillis();
		for (int i = from; i < to; i++) {
			if (i % 1000 == 0) {
				System.out.println("already spell:" + i + " ,spend time=" + (System.currentTimeMillis() - start));
			}
			...
			try {
				...
				try {
					...
					try {
						...
						System.out.println("sepell exception:" + i);
						e.printStackTrace();
						continue;
					}
				}
			...
			if (contents.size() < 1) {
				System.out.println("sepell empty:" + i);
				continue;
			}
